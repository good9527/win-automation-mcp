...
PS_OCR_SCRIPT = """$ErrorActionPreference = "Stop"
try {
    [void][Windows.Security.Credentials.PasswordVault, Windows.Security.Credentials, ContentType = WindowsRuntime]
    [void][Windows.Storage.StorageFile, Windows.Storage, ContentType = WindowsRuntime]
    [void][Windows.Storage.Streams.FileRandomAccessStream, Windows.Storage, ContentType = WindowsRuntime]
    ...
    function Await-WinRT ($asyncOp, $resultType) {
        ...
        try {
            $genericMethod = $asTaskGeneric.MakeGenericMethod($resultType)
            ...
            $asTaskProgressGeneric = ([System.WindowsRuntimeSystemExtensions].GetMethods() | Where-Object { 
                $_.Name -eq 'AsTask' -and $_.IsGenericMethod -and $_.GetGenericArguments().Count -eq 2 -and $_.GetParameters().Count -eq 1 -and $_.GetParameters()[0].ParameterType.Name -eq 'IAsyncOperationWithProgress`2' 
            })[0]
            $progressType = [System.UInt64]
            $genericMethod = $asTaskProgressGeneric.MakeGenericMethod($resultType, $progressType)
            $task = $genericMethod.Invoke($null, @($asyncOp))
            [void]$task.Wait(-1)
            return $task.Result
        }
    ...
    $file = Get-Item $tempPath
    $op1 = [Windows.Storage.StorageFile]::GetFileFromPathAsync($file.FullName)
    $storageFile = Await-WinRT $op1 ([Windows.Storage.StorageFile])
    
    $op2 = $storageFile.OpenAsync([Windows.Storage.FileAccessMode]::Read)
    ...
    $convertedBitmap = [Windows.Graphics.Imaging.SoftwareBitmap]::Convert($softwareBitmap, [Windows.Graphics.Imaging.BitmapPixelFormat]::Bgra8, [Windows.Graphics.Imaging.BitmapAlphaMode]::Premultiplied)

    $engine = $null
    if ([Windows.Media.Ocr.OcrEngine]::IsLanguageSupported("zh-Hans-CN")) {
    ...
    $result = Await-WinRT $op5 ([Windows.Media.Ocr.OcrResult])
...
@server.tool()
async def click_text(text: str, hwnd: Optional[int] = None) -> str:
    """Find and click a specific word or phrase in a window using native offline OCR.
    This enables robust, coordinate-free clicks on any button or text link!

    Args:
        text: The target text to search and click (supports Chinese and English)
        hwnd: Window handle. If not provided, falls back to the active target.
    """
    try:
        hwnd = _resolve_target(hwnd)
        ...
        # 1. Capture window screenshot (logical dimensions to ensure no black borders)
        png_data = _capture_window_screenshot(hwnd, max_width=1920)
        
        import tempfile
        temp_dir = tempfile.gettempdir()
        img_path = os.path.join(temp_dir, f"win_auto_ocr_{int(time.time())}.png")
        with open(img_path, "wb") as f:
            f.write(png_data)

        # 2. Write temp PowerShell script
        ps_script_path = os.path.join(temp_dir, "win_auto_ocr_runner.ps1")
        with open(ps_script_path, "w", encoding="utf-8") as f:
            f.write(PS_OCR_SCRIPT)

        # Output JSON file path
        output_json_path = os.path.join(temp_dir, f"win_auto_ocr_{int(time.time())}.json")

        # 3. Run PowerShell script
        cmd = [
            "powershell", "-NoProfile", "-ExecutionPolicy", "Bypass",
            "-File", ps_script_path, img_path, output_json_path
        ]
        ...
        # Clean up temp image and script
        ...
        if proc.returncode != 0 or not os.path.exists(output_json_path):
            ...
            return f"OCR execution failed: {stderr}"

        # 4. Load recognized words
        ...
        # 5. Search for target text
        target_normalized = text.strip().lower()
        matched_word = None
        
        # Method A: Direct exact or substring match in single word
        for item in data:
            item_text = item["text"].strip().lower()
            if target_normalized in item_text or item_text in target_normalized:
                matched_word = item
                break

        # Method B: Multi-word search (concatenated search for phrases)
        if not matched_word and len(data) > 0:
            full_str = "".join([w["text"] for w in data]).lower()
            idx = full_str.find(target_normalized)
            if idx != -1:
                char_count = 0
                for item in data:
                    item_len = len(item["text"])
                    if char_count <= idx < char_count + item_len:
                        matched_word = item
                        break
                    char_count += item_len

        if not matched_word:
            recognized_sample = ", ".join([f"'{w['text']}'" for w in data[:25]])
            return f"Target text '{text}' not found in OCR results. Top recognized words: {recognized_sample}"

        # 6. Click the center of the matched word
        window_rect = _get_window_rect(hwnd)
        win_w = window_rect.right - window_rect.left
        win_h = window_rect.bottom - window_rect.top
        ...
        # Center of matched word in logical screenshot space
        cx_logical = matched_word["x"] + matched_word["w"] / 2
        cy_logical = matched_word["y"] + matched_word["h"] / 2

        # Scale to physical window bounds
        real_x = int(cx_logical * win_w / log_w)
        real_y = int(cy_logical * win_h / log_h)
        ...
        return f"Found '{matched_word['text']}' at window client ({int(cx_logical)}, {int(cy_logical)}) -> Screen ({screen_x}, {screen_y}). Clicked successfully!"
